...
class LindormDataSource(DataSourceProvider):
    """Lindorm 数据源适配器 — 阿里云 Lindorm 宽表引擎 (MySQL 协议兼容)

    Lindorm 的 SQL 引擎兼容 MySQL 协议，复用 aiomysql 进行连接。
    额外支持 Lindorm 特有的时序/宽表/搜索索引能力。
    """
    ...
    async def connect(self, config: DataSourceConfig) -> None:
        ...
        self._conn = await aiomysql.connect(
            ...
            connect_timeout=10, charset=config.extra_params.get("charset", "utf8mb4"),
        )
    ...
    async def test_connection(self, config: DataSourceConfig) -> dict:
        ...
        try:
            ...
            conn.close()
            return {"success": True, "version": "Lindorm", "table_count": len(tables), "tables": tables}
        ...
        async with conn.cursor() as cur:
            ...
            for c in cols:
                ...
                columns.append({
                    ...
                    "is_primary": is_pk, "default": str(c[3]) if c[3] is not None else None,
                    "comment": c[4] or "",
                })
                if is_pk:
                    pk_col = c[0]
    ...
    async def fetch_data(
        ...
        async with conn.cursor() as cur:
            ...
            return [dict(zip(col_names, r)) for r in await cur.fetchall()]
